...
public interface StickerRepository extends JpaRepository<Sticker, Long> {
    ...
    @Query(value = "select s.sticker_id as id, s.name as sticker, s.url as url from stickers s " +
            ...
            "where r.place_id= :id group by rs.sticker_id order by count(rs.sticker_id) DESC limit 2", nativeQuery = true)
    List<StickerRes> findTop2Stickers(@PathVariable("id") Long id);
    ...
    List<StickerRes> findStickers(@Param("id") Long id);
}
